...
$(document).ready(function( )
{
	...
	$("#frmSearch").submit(function( )
	{
		...
		if ($("#frmSearch #Keywords").val( ) != "" || $("#frmSearch #Package").val( ) != "" || $("#frmSearch #Province").val( ) != "" || $("#frmSearch #District").val( ) != "" )
		{
			$.post("ajax/get-financial-progress.php",
				$("#frmSearch").serialize( ),
				...
				function (sResponse)
				{
					...
					$("#Summary").html(sParams[0]);
					$("#AccrualsReport").html(sParams[1]);
				},
...
//			$("#Statistics").hide("blind")
	...
	$(document).on("click", "#Map #ActiveCount, #Map #CompletedCount, #Map #AdoptedCount", function( )
	{
        ...
        $(document).on("click", ".icnSchools", function(event)
	{
		var iContractor = this.id;
                var sSchools = $(this).attr("rel");
                var sToDate  = $(this).attr("date");
		$.colorbox({ href:("list-wcc-schools.php?ContractorId=" + iContractor + "&Schools=" + sSchools+ "&ToDate=" + sToDate), width:"85%", height:"70%", iframe:true, opacity:"0.50", overlayClose:false });

		event.stopPropagation( );
	});
        
        $(document).on("click", ".icnCompSchools", function(event)
	{
		var iContractor = this.id;
                var sSchools = $(this).attr("rel");
                var sToDate  = $(this).attr("date");
		$.colorbox({ href:("list-fpc-schools.php?ContractorId=" + iContractor + "&Schools=" + sSchools+ "&ToDate=" + sToDate), width:"60%", height:"70%", iframe:true, opacity:"0.50", overlayClose:false });

		event.stopPropagation( );
	});
        
        $(document).on("click", ".icnDocuments", function(event)
	{
		var iContractor = this.id;
                var sSchools        = $(this).attr("rel");
                var sDownloadType   = $(this).attr("dtype");
                var sToDate         = $(this).attr("date");
		$.colorbox({ href:("download-documents.php?ContractorId=" + iContractor + "&Schools=" + sSchools +  "&DType=" + sDownloadType+  "&ToDate=" + sToDate), width:"50%", height:"50%", iframe:true, opacity:"0.50", overlayClose:false });

		event.stopPropagation( );
	});
        
        $(document).on("click", ".icnZipFile", function(event)
	{
		var iDistrict = this.id;
                var sSchools = $(this).attr("rel");
                document.location = ($(this).attr("url") + "?District=" + iDistrict + "&Schools=" + sSchools );
	});
...
function showSchoolStatus(iSchool)
{
	...
	$.post("ajax/get-school-status.php",
		...
		function (sResponse)
		{
            ...
            $("#FinancialProgress").html(sHtml[3]);
