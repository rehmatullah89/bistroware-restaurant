...
//			$("#Statistics").hide("blind")
	...
	$(document).on("click", "#Map #Status ul li.subStages", function( )
	{
		...
		$.post("ajax/get-school-stage-status.php",
			{ School:iSchool, Stage:iStage, Block:iBlock, Type:sType },
	...
	$(document).on("click", "#Map #ActiveCount, #Map #CompletedCount, #Map #QualifiedCount, #Map #AdoptedCount", function( )
	{
		document.location = ($(this).attr("rel") + "?" + $("#frmSearch").serialize( ));
	});	
	
	
	$(document).on("click", "#SchoolBlocks li div", function( )
	{
		$("#BlockWiseTimeline").html('<center><img src="images/waiting.gif" width="66" height="66" alt="" title="" vspace="150" /></center>');
		
		var iSchool = $(this).attr("school");
		var iBlock  = $(this).attr("block");
		
			
		$.post("ajax/get-school-block-timeline.php",
			{ School:iSchool, Block:iBlock },

			function (sResponse)
			{
				var sParams = sResponse.split("|-|");
				
				$("#BlockWiseTimeline").html(sParams[0]);
				$("#GeoTimelineOverall").html(sParams[1]);
