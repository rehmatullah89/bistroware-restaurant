...
<?
	if ($iTotalRecords <= 50)
	{
		$sSQL = "SELECT id, category_id, tax_id, cooking_time, store_id, name, price, cost, picture, status, created_at, modified_at,
		...
		for ($i = 0; $i < $iCount; $i ++)
		{
			...
			$iCategory   = $objDb->getField($i, "category_id");
                        $iMealId     = $objDb->getField($i, "meal_id");
...
?>
		        <tr id="<?= $iId ?>">
		          ...
		          <td><?= $sProduct ?></td>
                          <td><?= getDbValue("GROUP_CONCAT(meal SEPARATOR ', ')", "tbl_meals", "id IN ($iMealId)") ?></td>
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
			  <tr valign="top">
				<td width="600">
				  ...
				  <label for="ddMealType">Meal Type</label>

				  <div>
                                      <select name="ddMealType[]" id="ddMealType" multiple>
					  <option value=""></option>
<?
		foreach ($sMealsList as $iMeal => $sMeal)
		{
?> 
                                          <option value="<?= $iMeal ?>"<?= ((@in_array($iMeal, IO::getArray('ddMealType'))) ? ' selected' : '') ?>><?= $sMeal ?></option>
<?
		}
?>
				    </select>
...
?>
				  ...
				  <div><input type="text" name="txtCost" id="txtCost" value="<?= IO::strValue('txtCost', true) ?>" maxlength="20" size="8" class="textbox" /></div>
