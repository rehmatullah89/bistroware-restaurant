...
<?
                                    ...
                                    for ($i = 0; $i < $iCount; $i ++)
                                    {
                                            ...
                                            if(@in_array($sStatus, array('F','V','VP','VC')))
                                            {
                                                $iPrice = 0;
                                                $iNetPrice = 0;
                                            }
                                            ...
                                            $sInfo = ("<b>Void ".(@in_array($sStatus, array('R', 'RC'))?'Requested ':'')."By:</b><br />{$sVoidBy}<br />".formatDate($sVoidAt, "@{$_SESSION['DateFormat']} {$_SESSION['TimeFormat']}")."<br />");
?>
                                      <tr>
                                          <td><?=$i+1?></td>
                                          <td id="itemId">
...
?>
                                          ...
                                          <td>
                                           ...
                                           <td>
                                               <input type="text" size="5" name="txtPrice[]" id="txtPrice<?=$i?>" value="<?=$iNetPrice?>" class="textbox" readonly />
                                          </td> 
                                          <td>
                                              <select name="ddStatus[]" id="ddStatus_<?=$i?>" onchange="UpdatePrice(this.id, this.value)" style="width:100px; display:inline-block;">
                                                  ...
                                                  <option value="P" <?=($sStatus == 'P'?'selected':'')?>>Request Void on Pending Item</option>
                                                  <option value="PC" <?=($sStatus == 'PC'?'selected':'')?>>Request Void after Item Completion</option>
                                                  <option value="R" <?=($sStatus == 'R'?'selected':'')?>>Void Rejecteded on Pending Item</option>
                                                  <option value="RC" <?=($sStatus == 'RC'?'selected':'')?>>Void Rejecteded on Item Completion</option>
                                                  <option value="VP" <?=($sStatus == 'VP'?'selected':'')?>>Voided on Item Pending</option>
                                                  <option value="VC" <?=($sStatus == 'VC'?'selected':'')?>>Voided after Item Completion</option>
                                              </select>
<?
...
<!--
   ...
   function UpdatePrice(str, SelectedValue)
   {
        ...
        if(SelectedValue == 'V' || SelectedValue == 'VP' || SelectedValue == 'VC')
        {
            document.getElementById("txtPrice"+res[1]).value = 0;
            document.getElementById("txtQuantity_"+res[1]).value = 0;
        }
        
        else if(SelectedValue == 'A' || SelectedValue == 'R' || SelectedValue == 'P' || SelectedValue == 'PC' || SelectedValue == 'RC' || SelectedValue == 'C')
        {
