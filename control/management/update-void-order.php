	...
	if ($_SESSION["Flag"] == "")
	{
		$objDb->execute("BEGIN");
                ...
                foreach($iDetailIds as $iKey => $iId)
                {
                    ...
                    $iVoidReason= (int)trim($iVoidReasons[$iKey]);
                    $sRemarks   = trim($iRemarks[$iKey]);

                    if($iId > 0)
                    {
                        
                        $sSQL = "UPDATE tbl_order_details SET  status = '$sStatus', void_reason_id ='$iVoidReason', void_remarks = '$sRemarks', void_by='{$_SESSION['AdminId']}', void_at= NOW( )
