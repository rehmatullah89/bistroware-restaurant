	...
	if ($_SESSION["Flag"] == "")
	{
		...
		$bFlag = $objDb->execute($sSQL);
                ...
                if ($bFlag == true)
                {
                    ...
                    $iManufacts     = IO::getArray("txtManufac"); 
                    ...
                    foreach($iIdsArray as $iKey => $iId)
                    {
                        ...
                        $iManufac       = trim($iManufacts[$iKey]);
                        $iLocation      = trim($iLocations[$iKey]);
                        $iSupplier      = trim($iSuppliers[$iKey]);
                        $iWeight        = (float)trim($iWeights[$iKey]);
                        $iCountry       = trim($iCountries[$iKey]);

                        $iExpiry        = ($iExpiry == ""?"0000-00-00":$iExpiry);
                        $iManufac       = ($iManufac == ""?"0000-00-00":$iManufac);
