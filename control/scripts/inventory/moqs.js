...
$(document).ready(function( )
{
	...
	if (parseInt($("#TotalRecords").val( )) > 50)
	{
		objTable = $("#DataGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/inventory/get-moqs.php",
											 ...
											 fnDrawCallback  : function( )
											   ...
											   fnInitComplete  : function( )
																 {
																	$.post("ajax/inventory/get-moq-filters.php",
																		   {},
		...
		if (sMoqs != "")
		{
			$("#ConfirmMultiDelete").dialog( { resizable : false,
											   ...
											   buttons   : { "Delete" : function( )
															{
															 $.post("ajax/inventory/delete-moq.php",
																{ Moqs:sMoqs },
	...
	$(document).on("click", ".icnEdit", function(event)
	{
		...
		$.colorbox({ href:("inventory/edit-moq.php?MoqId=" + iMoqId + "&Index=" + iIndex), width:"40%", height:"70%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", ".icnView", function(event)
	{
		...
		$.colorbox({ href:("inventory/view-moq.php?MoqId=" + iMoqId), width:"40%", height:"70%", iframe:true, opacity:"0.50", overlayClose:true });
...
function updateRecord(iMoqId, iRow, sFields)
{
	if (parseInt($("#TotalRecords").val( )) <= 50)
	{
		$("#DataGrid thead tr th").each(function(iIndex)
