...
$(document).ready(function( )
{
	...
	$("#frmRecord").submit(function( )
	{
		...
		if (!objFV.validate("txtUnit", "B", "Please enter the Unit."))
			return false;
	...
	if (parseInt($("#TotalRecords").val( )) > 50)
	{
		objTable = $("#DataGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/inventory/get-units.php",
											 ...
											 fnDrawCallback  : function( )
											   ...
											   fnInitComplete  : function( )
																 {
																	$.post("ajax/inventory/get-unit-filters.php",
																		   {},
		...
		if (sUnits != "")
		{
			$("#ConfirmMultiDelete").dialog( { resizable : false,
											   ...
											   buttons   : { "Delete" : function( )
															{
															 $.post("ajax/inventory/delete-unit.php",
																{ Units:sUnits },
	...
	$(document).on("click", ".icnEdit", function(event)
	{
		...
		$.colorbox({ href:("inventory/edit-unit.php?UnitId=" + iUnitId + "&Index=" + iIndex), width:"40%", height:"40%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", ".icnView", function(event)
	{
		...
		$.colorbox({ href:("inventory/view-unit.php?UnitId=" + iUnitId), width:"40%", height:"40%", iframe:true, opacity:"0.50", overlayClose:true });
...
function updateRecord(iUnitId, iRow, sFields)
{
	if (parseInt($("#TotalRecords").val( )) <= 50)
	{
		$("#DataGrid thead tr th").each(function(iIndex)
