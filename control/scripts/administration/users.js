...
$(document).ready(function( )
{
	...
	if (parseInt($("#TotalRecords").val( )) > 50)
	{
		objUserTable = $("#UserGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/administration/get-users.php",
											   ...
											   fnInitComplete  : function( )
													 {
														$.post("ajax/administration/get-user-filters.php",
															   {},
		...
		if (sUsers != "")
		{
			$("#ConfirmUserMultiDelete").dialog( { resizable : false,
						           ...
						           buttons   : { "Delete" : function( )
									            {
											     $.post("ajax/administration/delete-user.php",
												    { Users:sUsers },
	...
	$(document).on("click", "#UserGrid .icnToggle", function(event)
	{
		...
		$.post("ajax/administration/toggle-user-status.php",
			{ UserId:objIcon.id },
	...
	$(document).on("click", "#UserGrid .icnEdit", function(event)
	{
		...
		$.colorbox({ href:("administration/edit-user.php?UserId=" + iUserId + "&Index=" + iIndex), width:"90%", height:"80%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", "#UserGrid .icnView", function(event)
	{
		...
		$.colorbox({ href:("administration/view-user.php?UserId=" + iUserId), width:"90%", height:"80%", iframe:true, opacity:"0.50", overlayClose:true });
	...
	$("#txtTitle").blur(function( )
	{
		...
		$.post("ajax/administration/check-user-type.php",
			{ Title:$("#txtTitle").val( ) },
	...
	$("#BtnTypeMultiDelete").click(function(event)
	{
		...
		if (sTypes != "")
		{
			$("#ConfirmTypeMultiDelete").dialog( { resizable : false,
						           ...
						           buttons   : { "Delete" : function( )
									            {
											     $.post("ajax/administration/delete-user-type.php",
												    { Types:sTypes },
	...
	$(document).on("click", "#TypeGrid .icnToggle", function(event)
	{
		...
		$.post("ajax/administration/toggle-user-type-status.php",
			{ TypeId:objIcon.id },
	...
	$(document).on("click", "#TypeGrid .icnEdit", function(event)
	{
		...
		$.colorbox({ href:("administration/edit-user-type.php?TypeId=" + iTypeId + "&Index=" + iIndex), width:"80%", height:"85%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", "#TypeGrid .icnView", function(event)
	{
		...
		$.colorbox({ href:("administration/view-user-type.php?TypeId=" + iTypeId), width:"80%", height:"85%", iframe:true, opacity:"0.50", overlayClose:true });
