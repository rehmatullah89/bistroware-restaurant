...
$(document).ready(function( )
{
	...
	if (parseInt($("#TotalRecords").val( )) > 50)
	{
		objTable = $("#DataGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/management/get-audits.php",
											 ...
											 fnDrawCallback  : function( )
											   ...
											   fnInitComplete  : function( )
																 {
																	$.post("ajax/management/get-audit-filters.php",
																		   {},
		...
		if (sAudits != "")
		{
			$("#ConfirmMultiDelete").dialog( { resizable : false,
											   ...
											   buttons   : { "Delete" : function( )
															{
															 $.post("ajax/management/delete-audit.php",
																{ Audits:sAudits },
	...
	{
		var iAuditId = this.id;
		var iIndex        = objTable.fnGetPosition($(this).closest('tr')[0]);

		$.colorbox({ href:("management/edit-audit.php?AuditId=" + iAuditId + "&Index=" + iIndex), width:"90%", height:"80%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", ".icnView", function(event)
	{
		...
		$.colorbox({ href:("management/view-audit.php?AuditId=" + iAuditId), width:"90%", height:"80%", iframe:true, opacity:"0.50", overlayClose:true });
...
function updateRecord(iAuditId, iRow, sFields)
{
	if (parseInt($("#TotalRecords").val( )) <= 50)
	{
		$("#DataGrid thead tr th").each(function(iIndex)
