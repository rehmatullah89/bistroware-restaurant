...
/*                if (!objFV.validate("ddReservation", "B", "Please Select a Reservation."))
			return false;   */ 
        ...
        $("#ddTable").on('change', function(){
            
                $.post("ajax/management/get-table-reservations.php",
                { TableId:$("#ddTable").val(),
                  OrderDate:$("#txtOrderDate").val(),  
                },
                function (sResponse)
                {
                    $("#ddReservation").html(sResponse);      
                },
                "text");
        ...
        $(document).on("click", "#DataGrid .icnToggle", function(event)
	{
		...
		var objRow  = objTable.fnGetPosition($(this).closest('tr')[0]);
                ...
                if($(objIcon).attr("void") > 0)
                {
                    alert("You can not change order status, unless voids get processed!");
                    return false;
                }

                if($(objIcon).attr("res") == 'Bumped' || $(objIcon).attr("res") == 'Active')
                {
                    $.colorbox({ href:("management/pay-order.php?OrderId=" + objIcon.id ), width:"450px", height:"400px", iframe:true, opacity:"0.50", overlayClose:false });
                     event.stopPropagation( );
                }
                else
                {
                    var result = confirm("Are you sure, you want to toggle status from completed to bumped?");
                    if (result) 
                    {
                        ...
                        $.post("ajax/management/toggle-order-status.php",
                            ...
                            function (sResponse)
                            {
                                    ...
                                    if (sParams[0] == "success")
                                    {
                                            ...
                                            {
                                                    objIcon.src = objIcon.src.replace("completed.png", "bumped.png");

                                                    objTable.fnUpdate("Bumped", objRow, iColumn, false);
                                                    $(objIcon).attr("res", "Bumped");
                                            }
                                    }
