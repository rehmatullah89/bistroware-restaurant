...
$(document).ready(function( )
{
	...
	$("#frmRecord").submit(function( )
	{
		...
		if (!objFV.validate("txtMeal", "B", "Please Enter the Meal Type."))
			return false;
	...
	if (parseInt($("#TotalRecords").val( )) > 50)
	{
		objTable = $("#DataGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/menu/get-meals.php",
											 ...
											 fnDrawCallback  : function( )
											   ...
											   fnInitComplete  : function( )
																 {
																	$.post("ajax/menu/get-meal-filters.php",
																		   {},
		...
		if (sMeals != "")
		{
			$("#ConfirmMultiDelete").dialog( { resizable : false,
											   ...
											   buttons   : { "Delete" : function( )
															{
															 $.post("ajax/menu/delete-meal.php",
																{ Meals:sMeals },
	...
	$(document).on("click", ".icnEdit", function(event)
	{
		...
		$.colorbox({ href:("menu/edit-meal.php?MealId=" + iMealId + "&Index=" + iIndex), width:"450", height:"300", iframe:true, opacity:"0.50", overlayClose:false });
        ...
        $(document).on("click", ".icnView", function(event)
	{
		...
		$.colorbox({ href:("menu/view-meal.php?MealId=" + iMealId), width:"450", height:"300", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", ".icnToggle", function(event)
	{
		...
		$.post("ajax/menu/toggle-meal-status.php",
			{ MealId:objIcon.id },
...
function updateRecord(iMealId, iRow, sFields)
{
        $("#DataGrid thead tr th").each(function(iIndex)
        {
