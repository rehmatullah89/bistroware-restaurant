<?
	...
	***********************************************************************************************
	...
	$sStatus     = IO::strValue("ddStatus");
        $sSendSms    = IO::strValue("ddSendSms");
        $sVoidSms    = IO::strValue("ddVoidSms");
        $sSalesSms  = IO::strValue("ddSalesSms");
	$sOldPicture = IO::strValue("Picture");
        $sCustomOpts = IO::getArray("cbCustomOpts");
        $sReportOpts = IO::getArray("cbReportOpts");
	$sPicture    = "";
	$sPictureSql = "";
        $iCustomOpts = implode(",", $sCustomOpts);
        $iReportOpts = implode(",", $sReportOpts);
        

	if ($sName == "" || $sMobile == "" || $sEmail == "" || $iType == 0 || $iRecords == 0 || $sTheme == "" || $sStatus == "")
	...
	if ($_SESSION["Flag"] == "")
	{
		...
		if ($sPassword != "")
			$sPasswordSql = ", password=PASSWORD('$sPassword') ";
                ...
                $iPrevType = getDbValue("type_id", "tbl_admins", "id='$iUserId'");
                
                if($iPrevType != $iType && $iType == 1)
                {
                    $iCustomOpts = implode(",", getList("tbl_custom_options", "id", "id"));
                }
		...
		$sSQL = "UPDATE tbl_admins SET type_id          = '$iType',
		                               ...
		                               email            = '$sEmail',
		                               mobile           = '$sMobile',
		                               custom_items     = '$iCustomOpts',
                                               reports          = '$iReportOpts',    
		                               records          = '$iRecords',
		                               theme            = '$sTheme',
                                               reservation_sms  = '$sSendSms',    
                                               void_sms         = '$sVoidSms',
                                               sales_sms        = '$sSalesSms',
		                               status           = '$sStatus'
                                                $sPictureSql
...
<?
			...
			if ($sUserRights["Delete"] == "Y" || $iType == 1)
			{
?>
