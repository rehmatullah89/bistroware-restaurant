<?
	...
	***********************************************************************************************
	...
	$sName      = $objDb->getField(0, "name");
        $iCustomOpts= $objDb->getField(0, "custom_items");
        $iReportOpts= $objDb->getField(0, "reports");
	...
	$sStatus    = $objDb->getField(0, "status");
        $sSendSms   = $objDb->getField(0, "reservation_sms");
        $sVoidSms   = $objDb->getField(0, "void_sms");
        $sSalesSms  = $objDb->getField(0, "sales_sms");
        
        $sCustomOpts = explode(",", $iCustomOpts);
        $sReportOpts = explode(",", $iReportOpts);
...
?>
		  ...
		  <div>
			<select name="ddStatus" id="ddStatus">
			  ...
			  <option value="D"<?= (($sStatus == 'D') ? ' selected' : '') ?>>Disabled</option>
                  ...
                  <label for="ddSendSms">Send Reservation Sms</label>

                  <div>
                    <input type="checkbox" name="ddSendSms" id="ddSendSms" value="Y" <?= (($sSendSms == "Y") ? "checked" : "") ?> />
                  </div>
                  
                  <div class="br10"></div>

                  <label for="ddVoidSms">Send Void Sms</label>

                  <div>
                    <input type="checkbox" name="ddVoidSms" id="ddVoidSms" value="Y" <?= (($sVoidSms == "Y") ? "checked" : "") ?> />
                  </div>
                         
                   <div class="br10"></div>

                    <label for="ddSalesSms">Sales Sms</label>

                    <div>
                      <input type="checkbox" name="ddSalesSms" id="ddSalesSms" value="Y" <?= (($sSalesSms == "Y") ? "checked" : "") ?> />
                    </div>
		  <br />
		  ...
		  <button id="BtnCancel">Cancel</button>
              ...
              <td>
                  <label for="ddCustomOpts">Custom Options <span>(<a href="#" rel="Check|customopt">Check All</a> | <a href="#" rel="Clear|customopt">Clear</a>)</span></label>

				  <div class="multiSelect" style="width:400px; height:auto;">
				    <table border="0" cellpadding="0" cellspacing="0" width="100%">
<?
		$sCustomSections = getList("tbl_custom_options", "id", "item");

		foreach ($sCustomSections as $iItem => $sItem)
		{
?>
					  <tr valign="top">
					    <td width="25"><input type="checkbox" class="customopt" name="cbCustomOpts[]" id="cbCustomOpt<?= $iItem ?>" value="<?= $iItem ?>" <?= ((@in_array($iItem, $sCustomOpts)) ? 'checked' : '') ?> /></td>
					    <td><label for="cbCustomOpt<?= $iItem ?>"><?= $sItem ?></label></td>
					  </tr>
<?
		}
?>
                                 ...
                                 <div class="br10"></div>
                                 <label for="ddReports">App Reports <span>(<a href="#" rel="Check|reportopt">Check All</a> | <a href="#" rel="Clear|reportopt">Clear</a>)</span></label>

				  <div class="multiSelect" style="width:400px; height:auto;">
				    <table border="0" cellpadding="0" cellspacing="0" width="100%">
<?
		$sReports = getList("tbl_reports", "id", "report", "id!=''", "position");

		foreach ($sReports as $iReport => $sReport)
		{
?>
					  <tr valign="top">
					    <td width="25"><input type="checkbox" class="reportopt" name="cbReportOpts[]" id="cbReportOpt<?= $iReport ?>" value="<?= $iReport ?>" <?= ((@in_array($iReport, $sReportOpts)) ? 'checked' : '') ?> /></td>
					    <td><label for="cbReportOpt<?= $iReport ?>"><?= $sReport ?></label></td>
					  </tr>
<?
		}
?>
...
<?
        if($iType == 1)
        {
            $sSQL = "SELECT ap.id, ap.module, ap.section, ar.`add`, ar.`edit`, ar.`delete`
			 ...
			 WHERE ap.id=ar.page_id AND ar.admin_id='{$_SESSION["AdminId"]}'
...
?>
			      ...
			      <tr class="<?= ((($i % 2) == 0) ? 'even' : 'odd') ?>">
				    ...
				    <td align="center"><input type="checkbox" name="cbAdd<?= $i ?>" id="cbAdd<?= $i ?>" value="Y" <?= (($sAddRights == "Y") ? "checked" : "") ?> <?= (($sAdd == "Y" || $iType == 1) ? "" : "disabled") ?> /></td>
				    <td align="center"><input type="checkbox" name="cbEdit<?= $i ?>" id="cbEdit<?= $i ?>" value="Y" <?= (($sEditRights == "Y") ? "checked" : "") ?> <?= (($sEdit == "Y"  || $iType == 1) ? "" : "disabled") ?> /></td>
				    <td align="center"><input type="checkbox" name="cbDelete<?= $i ?>" id="cbDelete<?= $i ?>" value="Y" <?= (($sDeleteRights == "Y") ? "checked" : "") ?> <?= (($sDelete == "Y"  || $iType == 1) ? "" : "disabled") ?> /></td>
