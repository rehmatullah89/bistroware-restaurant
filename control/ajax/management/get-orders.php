<?
	...
	***********************************************************************************************
	...
	$sColumns    = array('id', 'table_id', 'total_guests', 'waiter_id', 'created_at', 'TotalBill', 'TotalBill', 'status');
			...
			if (IO::strValue("bSortable_".IO::intValue("iSortCol_{$i}")) == "true")
			{
                                ...
                                else if ($sColumns[IO::intValue("iSortCol_{$i}")] == "waiter_id")
				{
					...
					$sOrderBy .= ("FIELD(waiter_id, {$sOrder}) ".strtoupper(IO::strValue("sSortDir_{$i}")).", ");
                                ...
                                else if($sColumns[IO::intValue("iSortCol_{$i}")] == "TotalBill")
                                {
                                    $sOrderBy .= "  _TotalBill ".strtoupper(IO::strValue("sSortDir_{$i}"))."  ";
                                }
                                else
	...
	if ($sKeywords != "")
	{
		$sConditions .= " AND (id LIKE '%{$sKeywords}%' 
                    OR waiter_id IN (SELECT id from tbl_admins WHERE `name` LIKE '%{$sKeywords}%') 
                    OR table_id IN (SELECT id from tbl_tables WHERE table_no LIKE '%{$sKeywords}%')
                )";
	}
	...
	$sSQL = "SELECT id, table_id, total_guests, waiter_id, remarks, status, created_at, modified_at,
                                (SELECT SUM(net_price) FROM tbl_order_details WHERE order_id=tbl_orders.id AND status NOT IN ('V','VC','VP','F')) AS _TotalBill,
                                (SELECT COUNT(1) FROM tbl_order_details WHERE order_id=tbl_orders.id AND status = 'FR') AS _FreeRequests,
                                (SELECT SUM(amount+credit) FROM tbl_payments WHERE order_id=tbl_orders.id)As _PaidAmt,
                                (SELECT COUNT(1) FROM tbl_order_details WHERE order_id=tbl_orders.id AND status IN ('P','PC')) AS _Voids,
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		$iId                = $objDb->getField($i, "id");
                ...
                $iPaidAmt           = $objDb->getField($i, "_PaidAmt");
		...
		$sOptions = (' <img class="icon details" id="'.$iId.'" src="images/icons/info.png" alt="" title="'.$sInfo.'" />');
                ...
                {
                    $sOptions = (' <img class="icnToggle" res="'.$sStatus.'" void="'.$iVoids.'" id="'.$iId.'" src="images/icons/'.(strtolower($sStatus)).'.png" alt="" title="'.$sInfo.'" />');
                }
		...
		$sOutput['aaData'][] = array( str_pad($iId, 5, '0', STR_PAD_LEFT),
		                              ...
		                              @utf8_encode($sCreatedAt),
                                              ...
                                              @utf8_encode(formatNumber($iPaidAmt, false)),
                                              @utf8_encode($sStatus),
		                              @utf8_encode($sOptions) );
	}
