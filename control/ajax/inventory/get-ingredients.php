<?
	...
	***********************************************************************************************
	...
	$sColumns    = array('id', 'title', 'unit_id', 'location_id', 'status');
	...
	if (IO::strValue("iSortCol_0") != "")
	{
		...
		for ($i = 0 ; $i < IO::intValue("iSortingCols"); $i ++)
		{
			...
			{
				if ($sColumns[IO::intValue("iSortCol_{$i}")] == "unit_id")
				{
					$sFields = getList("tbl_units", "id", "id", "", "title");
					$sOrder  = @implode(",", $sFields);

					$sOrderBy .= ("FIELD(unit_id, {$sOrder}) ".strtoupper(IO::strValue("sSortDir_{$i}")).", ");
				}
                                else if ($sColumns[IO::intValue("iSortCol_{$i}")] == "location_id")
				{
					$sFields = getList("tbl_locations", "id", "id", "", "title");
					$sOrder  = @implode(",", $sFields);

					$sOrderBy .= ("FIELD(location_id, {$sOrder}) ".strtoupper(IO::strValue("sSortDir_{$i}")).", ");
				}
                                else
	...
	@list($iTotalRecords, $iPageCount, $iStart) = getPagingInfo("tbl_ingredients", $sConditions, $iPageSize, $iPageId);
	...
	$sIngredientsSQL = "id!='0'";

	
	$sOutput = array("sEcho"                => IO::intValue("sEcho"),
	                 "iTotalRecords"        => getDbValue("COUNT(1)", "tbl_ingredients", $sIngredientsSQL),
	...
	for ($i = 0; $i < $iCount; $i ++)
	{
		...
		$sOutput['aaData'][] = array( str_pad($iId, 5, '0', STR_PAD_LEFT),
		                              @utf8_encode($sTitle),
		                              @utf8_encode($sUnitsList[$iUnitId]),
		                              @utf8_encode($sLocationsList[$iLocationId]),
		                              @utf8_encode($sStatus),
		                              $sOptions );
	}
