		...
		else
		{
			...
			else
			{
				...
				foreach ($sStagesList as $iParent => $sParent)
				{
					...
					$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iParent' AND `type`='$sBlockType' ORDER BY name";
					$objDb->query($sSQL);
					...
					for ($i = 0; $i < $iCount; $i ++)
					{
						...
						$sChildStages = getDbValue("GROUP_CONCAT(id SEPARATOR ',')", "tbl_stages", "status='A' AND parent_id='$iStage' AND `type`='$sBlockType'");
						...
						{
							$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iStage' AND `type`='$sBlockType' ORDER BY name";
							$objDb2->query($sSQL);
