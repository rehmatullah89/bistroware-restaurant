<?
	...
	***********************************************************************************************
	...
	$sEngineerOnSite     = IO::strValue("EngineerOnSite");
	$sContractorEngineer = IO::strValue("ContractorEngineer");
	$iCdcMember          = IO::intValue("CdcMember");
	$sCcsiMembersOnSite  = IO::strValue("CcsiMembersOnSite");
	$sCcsiMembers        = IO::strValue("CcsiMembers");
	$sHseMeasures        = IO::strValue("HseMeasures");
	$sRecordKeeping      = IO::strValue("RecordKeeping");
	$sDailyReport        = IO::strValue("DailyReport");
	$sHouseKeeping       = IO::strValue("HouseKeeping");
	$sSiteStatus         = IO::strValue("SiteStatus");
	$sSiteProgress       = IO::strValue("SiteProgress");
	$sDelayDuration      = IO::strValue("DelayDuration");
	$sSiteDead           = IO::strValue("SiteDead");
	$sInActiveDuration   = IO::strValue("InActiveDuration");
	$sNotifyAnyone       = IO::strValue("NotifyAnyone");
	$sNotifyUsers        = IO::strValue("NotifyUsers");
	...
	else
	{
		...
		else
		{
			...
			if ($sCcsiMembers != "")
			{
				$iCcsiMembers = @explode(",", $sCcsiMembers);
				
				for ($i = 0; $i < count($iCcsiMembers); $i ++)
				{
					if (strlen($iCcsiMembers[$i]) > 10)
						$iCcsiMembers[$i] = getDbValue("id", "tbl_school_members", "school_id='$iSchool' AND member_id='{$iCcsiMembers[$i]}'");
				}
				
				$sCcsiMembers = @implode(",", $iCcsiMembers);
			}			
			...
			else
			{
				...
				else
				{
					...
					$sSQL = "INSERT INTO tbl_inspections SET admin_id             = '$iUser',
															 ...
															 `date`               = '$sDate',
															 details              = '$sDetails',
															 engineer_on_site     = '$sEngineerOnSite',
															 contractor_engineer  = '$sContractorEngineer',
															 cdc_member_id        = '$iCdcMember',
															 ccsi_members_on_site = '$sCcsiMembersOnSite',
															 ccsi_members         = '$sCcsiMembers',
															 hse_measures         = '$sHseMeasures',
															 record_keeping       = '$sRecordKeeping',
															 daily_report         = '$sDailyReport',
															 house_keeping        = '$sHouseKeeping',
															 site_status          = '$sSiteStatus',
															 site_progress        = '$sSiteProgress',
															 delay_duration       = '$sDelayDuration',
															 site_dead            = '$sSiteDead',
															 inactive_duration    = '$sInActiveDuration',
															 notify_anyone        = '$sNotifyAnyone',
															 notify_users         = '$sNotifyUsers',
															 status               = '',
															 created_by           = '$iUser',
					...
					if ($bFlag == true)
					{
						...
						{
							$sSQL  = "UPDATE tbl_schools SET adopted='Y', qualified='Y', status='A' WHERE id='$iSchool'";
							$bFlag = $objDb->execute($sSQL, true, $iUser, $sName, $sEmail);
						}
					}
