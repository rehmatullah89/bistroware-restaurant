	...
	if ($objDb->query($sSQL) == true)
	{
		if ($objDb->getCount( ) == 1)
		{
			if ($objDb->getField(0, "status") == "A")
			{
				...
				$sSchoolTypesList   = getList("tbl_school_types", "id", "`type`");
				$sDocumentTypesList = getList("tbl_document_types", "id", "title", "status='A'");
				$sDesignationsList  = getList("tbl_school_member_designations", "id", "designation");
				
				$sDocumentTypesList[0] = "Statement of Requirements (SOR)";
				...
				$sSQL = "SELECT id, type_id, code, name, province_id, dropped, blocks, dangerous
						 FROM tbl_schools
				...
				for ($i = 0; $i < $iCount; $i ++)
				{
					...
					$sDangerous = $objDb->getField($i, "dangerous");


					$iSurveys             = getDbValue("COUNT(1)", "tbl_surveys", "school_id='$iSchool'");
					$iSors                = getDbValue("COUNT(1)", "tbl_sors", "school_id='$iSchool'");
					$sQualified           = (($iSurveys > 0) ? getDbValue("qualified", "tbl_surveys", "school_id='$iSchool'") : "Y");
					$sHss                 = ((@strpos($sSchoolTypesList[$iType], "HSS") !== FALSE) ? "Y" : "N");
					$sInspectionUsersList = getList("tbl_admins", "id", "name", "FIND_IN_SET(type_id, '3,5')");
					$sBlocksInfo          = array( );
					$sMembers             = array( );
					...
					for ($j = 0; $j < $iCount2; $j ++)
					{
						$iMember      = $objDb2->getField($j, "id");
						$iType        = $objDb2->getField($j, "type_id");
						$iDesignation = $objDb2->getField($j, "designation_id");
						$sMember      = $objDb2->getField($j, "name");
						$sPhone       = $objDb2->getField($j, "phone");
				...
				for ($i = 0; $i < $iCount; $i ++)
				{
					...
					$iAbledStudents         = getDbValue("SUM(COALESCE(boys_count, '0') + COALESCE(girls_count, '0'))", "tbl_survey_differently_abled_student_numbers", "survey_id='$iSurvey'");
					...
					$sToiletRamps           = getDbValue("IF(answer='Y', '1', '0')", "tbl_survey_answers", "question_id='43' AND survey_id='$iSurvey'");
					$sClassroomRamps        = getDbValue("IF(answer='Y', '1', '0')", "tbl_survey_answers", "question_id='42' AND survey_id='$iSurvey'");
					
					$iClassroomsProposed    = (@ceil($iAttendance / 40) - $iClassrooms);
					$sClassroomsProposed    = (($iClassroomsProposed > 0 && $iAttendance > 40) ? "{$iClassroomsProposed} New Required" : "No");
					
					$iNormalToiletsProposed = (@ceil($iAttendance / 80) - $iNormalToilets);
					$sNormalToiletsProposed = (($iNormalToiletsProposed > 0 && $iAttendance > 80) ? "{$iNormalToiletsProposed} New Required" : "No");
					
					$sDisableToiletsProposed = (($iAbledStudents > 0) ? '1' : '0');
					$sToiletRampsProposed    = (($sToiletRamps == "Y") ? "0" : "1");
					$sClassroomRampsProposed = (($sClassroomRamps == "Y") ? "0" : "1");
					...
					$sSurveySorsList[] = array("School"                 => $iSchool,
											   ...
											   "DisableToilets"         => $iDisableToilets,
											   "ToiletRamps"            => $sToiletRamps,
											   "ClassroomRamps"         => $sClassroomRamps,
					                           "ClassroomsProposed"     => $sClassroomsProposed,
											   "NormalToiletsProposed"  => $sNormalToiletsProposed,
											   "DisableToiletsProposed" => $sDisableToiletsProposed,
											   "ToiletRampsProposed"    => $sToiletRampsProposed,
											   "ClassroomRampsProposed" => $sClassroomRampsProposed);
				}
				

				$sFacilitiesList        = getList("tbl_sor_facilities", "id", "name", "status='A' AND hss_dangerous!='Y'");
				$sHssFacilitiesList     = getList("tbl_sor_facilities", "id", "name", "status='A' AND hss_dangerous='Y'");
				$sDistrictEngineersList = getList("tbl_admins", "id", "CONCAT(name, '|', provinces)", "type_id='6'");
				$sStageUnitsList        = getList("tbl_stages", "id", "unit", "status='A'");
				$sStageReasonsList      = getList("tbl_stages", "id", "failure_reasons", "status='A'");
				$sSsStagesList          = array( );
				$sDsStagesList          = array( );
				$sTsStagesList          = array( );
				$sBsStagesList          = array( );
				...
				foreach ($sStagesList as $iParent => $sParent)
				{
					...
					$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iParent' AND `type`='S' ORDER BY name";
					$objDb->query($sSQL);
					...
					for ($i = 0; $i < $iCount; $i ++)
					{
						...
						$sChildStages = getDbValue("GROUP_CONCAT(id SEPARATOR ',')", "tbl_stages", "status='A' AND parent_id='$iStage' AND `type`='S'");
						...
						{
							$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iStage' AND `type`='S' ORDER BY name";
							$objDb2->query($sSQL);
				...
				foreach ($sStagesList as $iParent => $sParent)
				{
					...
					$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iParent' AND `type`='D' ORDER BY name";
					$objDb->query($sSQL);
					...
					for ($i = 0; $i < $iCount; $i ++)
					{
						...
						$sChildStages = getDbValue("GROUP_CONCAT(id SEPARATOR ',')", "tbl_stages", "status='A' AND parent_id='$iStage' AND `type`='D'");
						...
						{
							$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iStage' AND `type`='D' ORDER BY name";
							$objDb2->query($sSQL);
				...
				foreach ($sStagesList as $iParent => $sParent)
				{
					...
					$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iParent' AND `type`='T' ORDER BY name";
					$objDb->query($sSQL);
					...
					for ($i = 0; $i < $iCount; $i ++)
					{
						...
						$sChildStages = getDbValue("GROUP_CONCAT(id SEPARATOR ',')", "tbl_stages", "status='A' AND parent_id='$iStage' AND `type`='T'");
						...
						{
							$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iStage' AND `type`='T' ORDER BY name";
							$objDb2->query($sSQL);
				...
				foreach ($sStagesList as $iParent => $sParent)
				{
					...
					$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iParent' AND `type`='B' ORDER BY name";
					$objDb->query($sSQL);
					...
					for ($i = 0; $i < $iCount; $i ++)
					{
						...
						$sChildStages = getDbValue("GROUP_CONCAT(id SEPARATOR ',')", "tbl_stages", "status='A' AND parent_id='$iStage' AND `type`='B'");
						...
						{
							$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iStage' AND `type`='B' ORDER BY name";
							$objDb2->query($sSQL);
				...
				foreach ($sStagesList as $iParent => $sParent)
				{
					...
					$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iParent' AND `type`='R' ORDER BY name";
					$objDb->query($sSQL);
					...
					for ($i = 0; $i < $iCount; $i ++)
					{
						...
						$sChildStages = getDbValue("GROUP_CONCAT(id SEPARATOR ',')", "tbl_stages", "status='A' AND parent_id='$iStage' AND `type`='R'");
						...
						{
							$sSQL = "SELECT id FROM tbl_stages WHERE status='A' AND parent_id='$iStage' AND `type`='R' ORDER BY name";
							$objDb2->query($sSQL);
				...
				$aResponse['Designations']      = $sDesignationsList;
				$aResponse['InspectionUsers']   = $sInspectionUsersList;
				...
				$aResponse['Facilities']        = $sFacilitiesList;
				$aResponse['HssFacilities']     = $sHssFacilitiesList;
				
				$aResponse['DocumentTypes']     = $sDocumentTypesList;
