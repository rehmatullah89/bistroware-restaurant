<?
	...
	***********************************************************************************************
	...
	if ($iFloor <= 0 || $iTable <= 0 || $sName == "" || $sPhone == "" || $iGuests <= 0 || $sResDate == "")
        ...
        $prevReservationTime = getDbValue("created_at", "tbl_reservations", "phone LIKE '$sPhone' OR (user_ip LIKE '$sUserIp' AND user_agent LIKE '$sUserAgent')", "id DESC");
        
        if($prevReservationTime != "")
        {
            $currentDateTime     = date("Y-m-d H:i:s");
            $HrsDiff    = round((strtotime($currentDateTime) - strtotime($prevReservationTime)) / 3600);
...
<script>
                    ...
                    parent.parent.showMessage("#GridMsg", "error", "You have already placed a reservation against this person, you can make new reservation after 2hrs of previous one for this person!"); 
...
<?
        ...
        {
            $currentDateTime     = date("Y-m-d H:i:s");
            $reservationSlot     = getDbValue("COUNT(1)", "tbl_reservations", "table_id = '$iTable' AND status != 'C' AND '$iStayDateTime' BETWEEN date_time AND end_time", "id DESC");
