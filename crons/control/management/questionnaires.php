<?
	...
	***********************************************************************************************
	...
	if ($_POST)
		@include("save-questionnaire.php");        
...
?>
  <script type="text/javascript" src="scripts/<?= $sCurDir ?>/questionnair.js"></script>
</head>
...
?>
      ...
      <div id="PageTabs">
	    <ul>
	      <li><a href="<?= $_SERVER['REQUEST_URI'] ?>#tabs-1"><b>Audit Questions</b></a></li>
<?
...
?>
	    ...
	    <div id="tabs-1">
	      ...
	      <div id="ConfirmMultiDelete" title="Delete Products?" class="hidden dlgConfirm">
	        <span class="ui-icon ui-icon-trash"></span>
	        Are you sure, you want to Delete the selected Questions Records?<br />
	      </div>
		  ...
		  <div class="dataGrid ex_highlight_row">
		    <input type="hidden" id="TotalRecords" value="<?= $iTotalRecords = getDbValue('COUNT(1)', 'tbl_audits_questionnaire', "status='A'") ?>" />
...
<?
	if ($iTotalRecords <= 50)
	{
		$sSQL = "SELECT id, question, status, created_at, modified_at,
						(SELECT name FROM tbl_admins WHERE id=tbl_audits_questionnaire.created_by) AS _CreatedBy,
						(SELECT name FROM tbl_admins WHERE id=tbl_audits_questionnaire.modified_by) AS _ModifiedBy
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
			  <tr valign="top">
				<td width="600">
                                  ...
                                  <label for="txtQuestion">Audit Question</label>
				  <div><input type="text" name="txtQuestion" id="txtQuestion" value="<?= IO::strValue("txtQuestion") ?>" size="20" class="textbox" /></div>
