...
?>
  ...
  <script type="text/javascript" src="scripts/<?= $sCurDir ?>/units.js"></script>
</head>
...
?>
      ...
      <div id="PageTabs">
	    <ul>
	      <li><a href="<?= $_SERVER['REQUEST_URI'] ?>#tabs-1"><b>Units</b></a></li>
<?
	if ($sUserRights["Add"] == "Y")
	{
?>
	      <li><a href="<?= $_SERVER['REQUEST_URI'] ?>#tabs-2">Add New Unit</a></li>
<?
	}
...
?>
	    ...
	    <div id="tabs-1">
	      ...
	      <div id="ConfirmDelete" title="Delete Inspection?" class="hidden dlgConfirm">
	        ...
	        Are you sure, you want to Delete this Unit Record?<br />
	      </div>

	      <div id="ConfirmMultiDelete" title="Delete Products?" class="hidden dlgConfirm">
	        <span class="ui-icon ui-icon-trash"></span>
	        Are you sure, you want to Delete the selected Units Records?<br />
	      </div>
		  ...
		  <div class="dataGrid ex_highlight_row">
		    <input type="hidden" id="TotalRecords" value="<?= $iTotalRecords = getDbValue('COUNT(1)', 'tbl_units', "status='A'") ?>" />
...
<?
	if ($iTotalRecords <= 50)
	{
		$sSQL = "SELECT id, title, code, status, created_at, modified_at,
						(SELECT name FROM tbl_admins WHERE id=tbl_units.created_by) AS _CreatedBy,
						(SELECT name FROM tbl_admins WHERE id=tbl_units.modified_by) AS _ModifiedBy
		        FROM tbl_units
                        ORDER BY position";
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
			  <tr valign="top">
				<td width="600">
				  ...
				  <div><input type="text" name="txtUnit" id="txtUnit" value="<?= IO::strValue("txtUnit") ?>" size="20" class="textbox" /></div>
												  
				  <div class="br10"></div>

				  <label for="txtCode">Code</label>
				  <div><input type="text" name="txtCode" id="txtCode" value="<?= IO::strValue('txtCode') ?>" maxlength="20" size="8" class="textbox" /></div>
                                  <div class="br10"></div>
