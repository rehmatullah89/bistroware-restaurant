...
$(document).ready(function( )
{
	...
	$("#frmRecord").submit(function( )
	{
		...
		if (!objFV.validate("txtTableTitle", "B", "Please enter the Table Title."))
			...
			return false;
                ...
                if (!objFV.validate("filePicture", "B", "Please select Table Picture."))
			return false;
	...
	if (parseInt($("#TotalRecords").val( )) > 50)
	{
		objTable = $("#DataGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/management/get-tables.php",
											 ...
											 fnDrawCallback  : function( )
											   ...
											   fnInitComplete  : function( )
																 {
																	$.post("ajax/management/get-table-filters.php",
																		   {},
		...
		if (sTables != "")
		{
			$("#ConfirmMultiDelete").dialog( { resizable : false,
											   ...
											   buttons   : { "Delete" : function( )
															{
															 $.post("ajax/management/delete-table.php",
																{ Tables:sTables },
	...
	$(document).on("click", ".icnEdit", function(event)
	{
		...
		$.colorbox({ href:("management/edit-table.php?TableId=" + iTableId + "&Index=" + iIndex), width:"50%", height:"80%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", ".icnView", function(event)
	{
		...
		$.colorbox({ href:("management/view-table.php?TableId=" + iTableId), width:"50%", height:"80%", iframe:true, opacity:"0.50", overlayClose:true });
...
function updateRecord(iTableId, iRow, sFields)
{
	if (parseInt($("#TotalRecords").val( )) <= 50)
	{
		$("#DataGrid thead tr th").each(function(iIndex)
		{
			...
			else if ($(this).text( ) == "Seats Capacity")
