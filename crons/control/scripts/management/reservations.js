...
{
        $(".opener").button({ icons:{ primary:'ui-icon-disk' } });
        
        $(".icnFloor").button({ icons:{ primary:'ui-icon-plusthick' } });
	...
	if (parseInt($("#TotalRecords").val( )) > 50)
	{
		objTable = $("#DataGrid").dataTable( { sDom            : '<"H"f<"toolbar"><"TableTools">>t<"F"ip>',
											   ...
											   bAutoWidth      : false,
											   bServerSide     : true,
											   sAjaxSource     : "ajax/management/get-reservations.php",
											 ...
											 fnDrawCallback  : function( )
											   ...
											   fnInitComplete  : function( )
																 {
																	$.post("ajax/management/get-reservation-filters.php",
																		   {},
		...
		if (sReservations != "")
		{
			$("#ConfirmMultiDelete").dialog( { resizable : false,
											   ...
											   buttons   : { "Delete" : function( )
															{
															 $.post("ajax/management/delete-reservation.php",
																{ Reservations:sReservations },
	...
	{
		var iReservationId = this.id;
		var iIndex        = objTable.fnGetPosition($(this).closest('tr')[0]);

		$.colorbox({ href:("management/edit-reservation.php?ReservationId=" + iReservationId + "&Index=" + iIndex), width:"600px", height:"80%", iframe:true, opacity:"0.50", overlayClose:false });
	...
	$(document).on("click", ".icnView", function(event)
	{
		...
		$.colorbox({ href:("management/view-reservation.php?ReservationId=" + iReservationId), width:"600px", height:"80%", iframe:true, opacity:"0.50", overlayClose:true });
...
function updateRecord(iReservationId, iRow, sFields)
{
	if (parseInt($("#TotalRecords").val( )) <= 50)
	{
		$("#DataGrid thead tr th").each(function(iIndex)
		{
			...
			else if ($(this).text( ) == "# of Guests")
				objTable.fnUpdate(sFields[3], iRow, iIndex);
                            
                        else if ($(this).text( ) == "Floor")
