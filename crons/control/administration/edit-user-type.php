<?
	...
	***********************************************************************************************
	...
	$sStatus = $objDb->getField(0, "status");
        $iCuOpts = $objDb->getField(0, "custom_opts");
        
        $sCuOpts = explode(",", $iCuOpts);
?>
...
<?
                               ...
                               {
?>
                              <tr  class="<?= ((($i % 2) == 0) ? 'even' : 'odd') ?>"><td><label for="cbCustomOpt<?= $iItem ?>"><?= $sItem ?></label></td><td><input type="checkbox" class="customopt" name="cbCustomOpts[]" id="cbCustomOpt<?= $iItem ?>" value="<?= $iItem ?>" <?= ((@in_array($iItem, $sCuOpts)) ? 'checked' : '') ?> /></td></tr>
<?
                                    $i++;
                               }
?>
